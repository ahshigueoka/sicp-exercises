...
(define sets-tests
  (test-suite
   ...
   (test-case
    ...
    (check-equal? (adjoin-set 'x '(z x y)) '(x z x y) "Adjoining to a set that has x"))
   (test-case
    "Empty intersection"
   ...
   (test-case
    ...
    (check-equal? (union-set '(3 9 5) '(4 9 0 3)) '(3 9 5 4 9 0 3) "Intersecting sets"))
   (test-case
    "Removing repetitions"
    (check-equal? (unique-elements '(3 9 5 4 9 0 3)) '(0 4 5 9 3) "Repetition removal"))))
